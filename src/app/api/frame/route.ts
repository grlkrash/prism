...
import { Message, getFrameMessage, validateFrameMessage } from '@farcaster/frame-sdk'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mbdResults.status === 'fulfilled' && mbdResults.value?.casts) {
      const culturalTokens = mbdResults.value.casts.filter(cast => 
        cast.aiAnalysis?.hasCulturalElements || 
        cast.aiAnalysis?.category?.toLowerCase().includes('art')
      )
      
      // Convert MBD casts to tokens
      const mbdTokens = await Promise.all(culturalTokens.map(async cast => {
